...
/// <summary>
/// Repository interface for TtsMessageHistory entities with history-specific operations.
/// </summary>
public interface ITtsMessageHistoryRepository : IRepository<TtsMessageHistory>
{
    /// <summary>
    /// Gets the most recent TTS messages played by a user in a specific guild.
    /// </summary>
    ...
    Task<IReadOnlyList<TtsMessageHistory>> GetRecentAsync(
        ulong userId,
        ulong guildId,
        int limit = 20,
    ...
    /// <summary>
    /// Gets all favorited TTS messages for a user in a specific guild.
    /// </summary>
    /// <param name="userId">Discord user ID.</param>
    /// <param name="guildId">Discord guild ID.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Read-only list of favorited history entries ordered by most recent first.</returns>
    Task<IReadOnlyList<TtsMessageHistory>> GetFavoritesAsync(
        ulong userId,
        ulong guildId,
        CancellationToken cancellationToken = default);
