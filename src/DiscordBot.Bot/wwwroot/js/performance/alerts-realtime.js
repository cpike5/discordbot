    ...
    function addAlertRow(container, alert) {
        ...
        row.innerHTML = `
            ...
            <div class="alert-actions flex items-center gap-2">
                <form method="post" action="/Admin/Performance/Alerts?handler=Acknowledge&id=${alert.id}">
                    <input type="hidden" name="__RequestVerificationToken" value="${token}" />
                    <button type="submit" class="acknowledge-btn btn btn-secondary btn-sm">Acknowledge</button>
                </form>
            </div>
        `;
