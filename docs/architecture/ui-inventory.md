...
above except the two Portal ones (`/blazor-smoke`, `/admin/blazor-smoke`, `/admin/blazor-probe`,
`/components`, and — one level down, via `@layout GuildLayout` — `/Guilds/{guildId}/blazor-probe`).
`/Portal/{guildId}/blazor-probe` opts into `PortalLayout` instead, which is not nested under
`MainLayout` (see that layout's own row below).
