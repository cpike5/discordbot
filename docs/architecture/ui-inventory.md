...
| `/Account/Login` | `Blazor/Pages/Account/Login.razor` (+ `Login.razor.cs`) | Static SSR, `[AllowAnonymous]`, `@layout EmptyLayout` (cluster 4c). Replaces `Pages/Account/Login.cshtml` + `LoginModel` — single centred card (the brand side panel was already removed on `main`). Email/password sign-in is `IPasswordSignInService` (`Services/Account/`), the Discord button and "Try again" resubmit post to the `POST /Account/PerformExternalLogin` minimal API (`AccountEndpointExtensions`), and both are wired behind the same `?authError=discord_unavailable\|discord_expired\|discord_unconfigured\|discord_error` contract and `returnUrl` sanitizing (`LocalUrl.IsLocal` + `ReturnUrlHelper.Sanitize`) the legacy page had. `EditForm FormName="login"` (a static SSR form needs `FormName`, unlike an interactive one) + `DataAnnotationsValidator` replace jQuery unobtrusive validation; the password-visibility toggle is the only surviving client script, `wwwroot/js/blazor/account.js`, loaded via `<HeadContent>`. The dead `LoginWith2fa` redirect branch (no 2FA flow exists) is removed — `SignInResult.RequiresTwoFactor` is treated as an ordinary failed attempt. |
| `/Account/LinkDiscord` | `Blazor/Pages/Account/LinkDiscord.razor` (+ `.razor.cs`) | Static SSR, `[Authorize]`, default `MainLayout`. Replaces `Pages/Account/LinkDiscord.cshtml` + `LinkDiscordModel` (cluster 4c). Two top-level states (linked/not linked), not three — "OAuth not configured" is a modifier of the not-linked state rather than its own branch, so bot verification (which never touches the OAuth client) stays reachable in web-only mode; the legacy page nested it under "OAuth is configured" for no functional reason. Every same-page mutation (unlink, refresh Discord data, initiate/verify/cancel bot verification) shares exactly one `<EditForm FormName="link-discord-actions">` (appearing in at most one of the two mutually exclusive branches), dispatched in `HandleFormActionAsync` by which submit button's `*Action` field posted, and calls `IDiscordLinkService` (`Services/Account/`) before redirecting to `?status=<key>[&detail=...]` — banner copy is centralised in `StatusBanner` (mirrors `Profile.razor`'s `status=saved\|error`, extended to ~15 keys); `detail` is forwarded only on a FAILURE redirect (a service's own error message), never on success, so a crafted URL can't put attacker text in a success banner — the one success case with dynamic text (`verify-code-success`'s "Welcome, {username}!") reads the linked `DiscordUsername` back from the database instead. Unlink is a `<details>/<summary>` two-step confirmation (no interactive `ConfirmModal` on a static SSR page); the OAuth challenge itself is a plain `<form>` posting to `POST /Account/PerformExternalLogin` with a hidden `returnUrl`, not a same-page handler. |
| `/Account/Privacy` | `Blazor/Pages/Account/Privacy.razor` (+ `.razor.cs`, `.razor.css`) | Static SSR, `[Authorize]`, default `MainLayout`. Replaces `Pages/Account/Privacy.cshtml` + `PrivacyModel` (cluster 4c) — calls `IConsentService`/`IUserDataExportService`/`IUserPurgeService` directly (no wrapper service; each mutation is already one call). **Two** named forms, not one: every consent Grant/Revoke and "Export My Data" share one `<EditForm FormName="privacy-actions">` (each consent toggle is itself a `<details>/<summary>` two-step confirmation, reproducing the legacy page's `confirmConsentToggle` JS dialog), while "Delete My Data" is a separate `<EditForm FormName="privacy-delete">` with its own bound model — deliberately not sharing `privacy-actions`, since HTML implicit submission on Enter in a text input fires the *first* submit button in its form, and typing into the same form as the consent buttons would silently toggle a consent instead of asking for a delete. The typed confirmation (`Confirmation` must equal the literal text `DELETE`) is checked in C# in the delete form's own submit handler, not `DataAnnotationsValidator` — a mismatch renders inline instead of redirecting; a real purge signs the session out (`SignInManager.SignOutAsync`) and redirects to `/landing`. Same `?status=<key>[&detail=...]` banner mechanism as `LinkDiscord`, including the success-never-carries-`detail` rule — `export-success` renders fixed, static copy rather than the legacy page's per-export record count/download link, since neither persists for the redirect's own GET to re-read. |
...
| `/Guilds/RatWatch/{guildId:long}` | `Blazor/Pages/Guilds/RatWatch/Index.razor` (+ `.razor.cs`) | `@rendermode InteractiveServer`, `[Authorize(Policy = "RequireAdmin")]`, `@layout GuildLayout`. Replaces `Pages/Guilds/RatWatch/Index.cshtml` + `IndexModel` — settings card (display/edit toggle), Settings/Analytics/Incidents sub-nav (`TabGroup`; Analytics is still a Razor Page, Incidents moved to Blazor in cluster 4d, see below), summary/analytics cards, watch stats + hall-of-shame leaderboard, and the recent-watches table. Cancel/end-vote are `ConfirmModal`-gated component methods calling `IRatWatchService.CancelWatchAsync`/`FinalizeVotingAsync` (replacing the legacy page's `OnPostCancelAsync`/`OnPostEndVoteAsync` handlers); the settings save reproduces the same timezone-non-blank/1-168-hour/1-60-minute validation `OnPostUpdateSettingsAsync` had. Reuses the legacy `RatWatchItemViewModel`/`RatLeaderboardEntryViewModel` (`ViewModels/Pages/RatWatchIndexViewModel.cs` — still alive, `Blazor/Pages/Guilds/RatWatch/Incidents.razor.cs` also depends on them) for status/badge derivation rather than reimplementing it. |
| `/Guilds` | `Blazor/Pages/Guilds/Index.razor` (+ `.razor.cs`) | `@rendermode InteractiveServer`, `[Authorize(Policy = "RequireModerator")]`, default `MainLayout` (top-level, not guild-scoped). Replaces `Pages/Guilds/Index.cshtml` + `IndexModel` (cluster 4d) — the guild list with search/status filter/sort/`PagedQuery` pagination, reusing `GuildListViewModel`/`GuildSummaryItem` unchanged. Per-row sync (any Moderator+) and Sync All (`Admin`/`SuperAdmin`, gated in the UI and the handler) both call `IGuildService` through `ScopedOperations`. The legacy mobile card's row link built `Url.Page("Details", new { id = guild.Id })` — `id` is not that page's route parameter name (`guildId` is), so it linked to `/Guilds/Details` with no id segment; fixed to the same `DetailsUrl` helper the desktop row uses. |
| `/Guilds/Details/{guildId:long}` | `Blazor/Pages/Guilds/Details.razor` (+ `.razor.cs`) | `@rendermode InteractiveServer`, `[Authorize(Policy = "RequireModerator")]` + `GuildAccess`, `@layout GuildLayout`, `@inherits GuildPageBase` — the guild dashboard, the "overview" nav tab. Replaces `Pages/Guilds/Details.cshtml` + `DetailsModel` (cluster 4d). `IGuildDetailsAggregator.BuildAsync` feeds six dashboard widgets (Scheduled Messages, Rat Watch, Reminders, Audio, Assistant, Members) rendered as real `Card`/`EmptyState`/`Badge`/`StatTile` markup, plus an Activity table of recent command logs — replacing the legacy `DashboardWidgetViewModel.BodyContent` raw-HTML-string approach. `CanEdit` comes from `GuildContext.CanEdit`; since `GuildLayout`'s `GuildHeader` carries no action slot, the page renders its own action bar (Sync + Edit Settings when `CanEdit`, plus a "More Actions" dropdown with a real Copy Guild ID action via `BrowserInterop.CopyToClipboardAsync` — the legacy page's own `moreActionsDropdown`/`copyToClipboard` script matched no markup anywhere, dead code from the start). `GuildPageBase.Dispose()` is now `virtual` so this page can release its own click-outside interop handle. |
| `/Guilds/FlaggedEvents/{guildId:long}` | `Blazor/Pages/Guilds/FlaggedEvents/Index.razor` (+ `.razor.cs`) | `@rendermode InteractiveServer`, `[Authorize(Policy = "RequireAdmin")]` + `GuildAccess`, `@layout GuildLayout`. Replaces `Pages/Guilds/FlaggedEvents/Index.cshtml` + `IndexModel` (cluster 4d) — rule type/severity/status/date filters, bulk-select toolbar, and per-row/bulk dismiss/acknowledge, all calling `IFlaggedEventService` directly (the legacy page's inline JS instead called the now-deleted `FlaggedEventsController`). Reuses the existing Tier 1b `SeverityBadge`/`RuleTypeIcon`/`StatusBadge` components. The reviewer id is resolved from the current user's `discord:user_id` claim (`ClaimsPrincipalExtensions.GetDiscordUserId`) — the legacy inline JS read a claim named `"DiscordId"`, which no authentication handler in this app ever issues, so it was always empty in production; the port resolves the real claim and refuses the action with a toast when it's absent. |
| `/Guilds/FlaggedEvents/{guildId:long}/{id:guid}` | `Blazor/Pages/Guilds/FlaggedEvents/Details.razor` (+ `.razor.cs`) | Same auth/layout as Index. Replaces `Pages/Guilds/FlaggedEvents/Details.cshtml` + `DetailsModel` (cluster 4d) — event summary, user info, a collapsible evidence panel, up to 10 prior events for the same user, and a sticky actions panel (Dismiss/Acknowledge/Take Action, the latter four canned reasons plus a custom-text `Modal`), all through `IFlaggedEventService`. Fixes the legacy post-dismiss redirect, which targeted the non-existent `/Guilds/FlaggedEvents/Index/{guildId}`, to the real list route. |
| `/Guilds/RatWatch/Incidents/{guildId:long}` | `Blazor/Pages/Guilds/RatWatch/Incidents.razor` (+ `.razor.cs`) | `@rendermode InteractiveServer`, `[Authorize(Policy = "RequireModerator")]` + `GuildAccess`, `@layout GuildLayout`. Replaces `Pages/Guilds/RatWatch/Incidents.cshtml` + `IncidentsModel` (cluster 4d) — the same Settings/Analytics/Incidents `TabGroup` sub-nav as `RatWatch/Index`, a `FilterPanel` (status multi-select, date range defaulting to the last 30 days when both are unset, accused/initiator user, min vote count, keyword), and the incident table with an on-demand detail `Modal` (`IRatWatchService.GetByIdAsync`, replacing the legacy `?handler=IncidentDetail` JSON endpoint). Read-only, matching `IncidentsModel` (no cancel/end-vote handler existed either). CSV export is now built server-side from the loaded page's rows (same columns, BOM included) via `BrowserInterop.DownloadFileAsync`, replacing the legacy client-side embedded-JSON export. |

**Account minimal-API endpoints** (`Extensions/AccountEndpointExtensions.cs`, `MapAccountEndpoints()`, all `[AllowAnonymous]`) replace the POST/GET handlers `Pages/Account/{Login,ExternalLogin,Logout}.cshtml.cs` used to carry, now that those three pages are gone (cluster 4c):

| Route | Purpose |
|-------|---------|
| `POST /Account/Logout` | Signs the current user out (audit-logged first), then `LocalRedirect`s to a sanitized `returnUrl` or `/landing`. Posted to by `MainNavbar.razor`'s and the legacy `_Navbar.cshtml`'s logout forms, and `Blazor/Pages/Account/AccessDenied.razor`'s "Sign Out" button — each a plain `<form>` with its own antiforgery token, not an `EditForm`. |
| `POST /Account/PerformExternalLogin` | Issues the Discord OAuth challenge (`Results.Challenge`) with a `RedirectUri` of `/Account/ExternalLogin/Callback?returnUrl=...`; redirects to `/Account/Login?authError=discord_unconfigured` if Discord OAuth isn't configured. Shared by `Login.razor`'s Discord button and `LinkDiscord.razor`'s "Link Discord Account" form. |
| `GET /Account/ExternalLogin/Callback` | The Discord OAuth callback (not to be confused with the unchanged `/signin-discord` middleware callback `AspNet.Security.OAuth.Discord` itself uses). Delegates to `IExternalLoginHandler` (`Services/Account/`) for the sign-in-or-link-or-create flow, then turns its outcome into a redirect. |
| `GET /Account/ExternalLogin` | Legacy no-handler GET — redirects to `/Account/Login`. |

Route strings are `DiscordBot.Bot.Extensions.AccountRoutes` constants (`Login`, `Logout`, `PerformExternalLogin`, `ExternalLoginCallback`, `Lockout`, `AccessDenied`, `LinkDiscord`, `Privacy`). Both minimal-API POST endpoints bind a `[FromForm]` parameter, which decorates them with `IAntiforgeryMetadata` (`RequiresValidation == true`) automatically — proven by `AccountEndpointExtensionsTests.MapAccountEndpoints_PostEndpointsRequireAntiforgery_GetEndpointsDoNot`, which builds the real endpoint data via `MapAccountEndpoints` on a `WebApplication` and asserts the metadata directly (the other handler-level tests in that file call the handlers as plain delegates and never construct an endpoint).
...
Cluster 4d ("Lists and settings", partial) ports five Admin pages. `/Admin/AuditLogs` and
`/Admin/MessageLogs` stay minimal-API redirects to `/Admin/Logs?tab=...` (`Extensions/LegacyRedirectExtensions.cs`, unchanged by this cluster).

| Route | File | Purpose |
| --- | --- | --- |
| `/Admin/Logs` | `Blazor/Pages/Admin/Logs/Index.razor` (+ `.razor.cs`), `Tabs/MessagesTab.razor`/`Tabs/AuditTab.razor` (+ `.razor.cs` each) | `RequireAdmin`. Replaces `Pages/Admin/Logs/Index.cshtml` + `IndexModel` (unifying the legacy Audit Logs/Message Logs tabs). `?tab=messages\|audit` (default messages) selects a `TabGroup Mode=Navigation`; a disabled "Application" tab is a placeholder. Each tab is its own component with its own `[SupplyParameterFromQuery]` filters (exact legacy names: `AuthorId`/`MessageGuildId`/`ChannelId`/`MessageSource`/`MessageStartDate`/`MessageEndDate`/`MessageSearchTerm`/`messagePageNumber`; `Category`/`Action`/`ActorId`/`TargetType`/`AuditGuildId`/`AuditStartDate`/`AuditEndDate`/`AuditSearchTerm`/`auditPageNumber`/`UserTimezone`) — `Services/Search/{MessageLogsSearchProvider,AuditLogsSearchProvider}` and `AuditLogCard.razor` link against them. `Autocomplete` search functions call `IMessageLogRepository`/`IGuildService`/`DiscordSocketClient` directly (no HTTP round trip; `AutocompleteController` stays for other consumers). Audit CSV export is a plain `<a href>` to the minimal-API `GET /api/admin/audit-logs/export` (`Extensions/AdminLogsEndpointExtensions.cs`, CSV built by `Services/AdminLogsCsvExporter.cs`) instead of a page handler. A Discord snowflake or nullable enum bound via `[SupplyParameterFromQuery]` is exposed as `string`/`int` (`AuthorIdQuery`, `CategoryQuery`, etc.) with a computed typed property — `QueryParameterValueSupplier` has no built-in `ulong`/arbitrary-`enum` support (same note as `Guilds/AudioModerationLog/Index.razor.cs`). |
| `/Admin/Notifications` | `Blazor/Pages/Admin/Notifications/Index.razor` (+ `.razor.cs`) | `RequireViewer`. Replaces `Pages/Admin/Notifications/Index.cshtml` + `IndexModel` (`PaginatedPageModel`, page size 25, `Blazor/Common/PagedQuery` query names). Bulk mark-read/delete and per-row toggle-read/delete call `INotificationService` directly through `ScopedOperations`, reloading the list in place — no `NotificationsController` round trip (that controller and `wwwroot/js/notification-history.js` retire with this page; no other consumer). Delete All is `ConfirmModal`-gated with `RequiredText="DELETE"`. `NotificationBell.razor` gained a one-line "View all notifications" footer link to this page. |
| `/Admin/BulkPurge` | `Blazor/Pages/Admin/BulkPurge/Index.razor` (+ `.razor.cs`) | `RequireSuperAdmin`. Replaces `Pages/Admin/BulkPurge.cshtml` + `BulkPurgeModel`. Entity-type cards, Preview/Execute call `IBulkPurgeService` through `ScopedOperations`, `ConfirmModal RequiredText="CONFIRM"`. The progress bar is real: subscribes to `IDashboardEventBus`'s `BulkPurgeProgressEvent` before the purge starts (not guild-scoped) and unsubscribes on completion — the legacy `#progress-container` markup was dead. |
| `/Admin/UserPurge` | `Blazor/Pages/Admin/UserPurge/Index.razor` (+ `.razor.cs`) | `RequireSuperAdmin`. Replaces `Pages/Admin/UserPurge.cshtml` + `UserPurgeModel`. GET-driven preview via `?DiscordUserId=`; a `_Anonymized`-suffixed category renders an "Anonymized" badge. `ConfirmModal RequiredText` is dynamic — the looked-up Discord user id itself, not a fixed word. |
| `/Admin/LlmUsage` | `Blazor/Pages/Admin/LlmUsage/Index.razor` (+ `.razor.cs`) | `RequireAdmin`. Replaces `Pages/Admin/LlmUsage.cshtml` + `LlmUsageModel`; retires `LlmUsageController` and `wwwroot/js/llm-usage.js`. Hero tiles, Cost by User (clickable rows open an in-page drill-down), By Model/By Mode/By Day, 7/30/90-day presets. The per-user drill-down is a paged component method straight over `ILlmUsageRepository.GetRecordsAsync` (callback-mode `Pagination`) instead of a client-side fetch to `api/admin/llm-usage/records`. Range default/clamp logic (`Services/LlmUsageRangeHelper.cs`) is the one surviving copy of what used to be duplicated between the controller and the page model. |
...
`LandingLayout` (static SSR, replaces `_LayoutLanding.cshtml` for `/landing`) landed with the landing page; `GuildLayout`/`PortalLayout` follow in the same phase. `/Account/Login` (cluster 4c) uses `EmptyLayout`, not `LandingLayout` — it's a single centred card, not the full landing chrome.
...
| Route | File | Purpose |
|-------|------|---------|
All `/account/*` pages moved to Blazor: `/account/access-denied`, `/account/lockout` and `/account/profile` in Phase 4
cluster 4a; `/account/login`, `/account/external-login`, `/account/logout`, `/account/link-discord` and
`/account/privacy` in cluster 4c — see "Blazor Routes (Phase 4, permanent)" above.
...
`/admin/logs`, `/admin/notifications`, `/admin/bulk-purge`, `/admin/user-purge` and `/admin/llm-usage`
moved to Blazor in Phase 4 cluster 4d — see "Blazor Routes (Phase 4, permanent)" above.

### Guild Pages (Per-Server Management)

Reminders, Scheduled Messages, and Feature Requests moved to Blazor in Phase 4 cluster 4b; the
guild list/dashboard and Flagged Events moved to Blazor in Phase 4 cluster 4d — see "Blazor Routes
(Phase 4, permanent)" above.
