...
| `POST /Account/PerformExternalLogin` | Issues the Discord OAuth challenge (`Results.Challenge`) with a `RedirectUri` of `/Account/ExternalLogin/Callback?returnUrl=...`; redirects to `/Account/Login?authError=discord_error` if Discord OAuth isn't configured. Shared by `Login.razor`'s Discord button and `Pages/Account/LinkDiscord.cshtml.cs`'s "Link Discord" action. |
...
Route strings are `DiscordBot.Bot.Extensions.AccountRoutes` constants (`Login`, `Logout`, `PerformExternalLogin`, `ExternalLoginCallback`, `Lockout`, `AccessDenied`).
| `/Account/LinkDiscord` | `Blazor/Pages/Account/LinkDiscord.razor` (+ `.razor.cs`) | Static SSR, `[Authorize]`, default `MainLayout`. Replaces `Pages/Account/LinkDiscord.cshtml` + `LinkDiscordModel` (cluster 4c). Two top-level states (linked/not linked), not three — "OAuth not configured" is a modifier of the not-linked state rather than its own branch, so bot verification (which never touches the OAuth client) stays reachable in web-only mode; the legacy page nested it under "OAuth is configured" for no functional reason. Every same-page mutation (unlink, refresh Discord data, initiate/verify/cancel bot verification) is an `<EditForm FormName="...">` calling the new `IDiscordLinkService` (`Services/Account/`) and redirecting to `?status=<key>[&detail=...]`, whose banner copy is centralised in `StatusBanner` (mirrors `Profile.razor`'s `status=saved\|error`, extended to ~15 keys; a handful with genuinely dynamic legacy text — a service error message, the linked Discord username — carry it through `detail` instead of duplicating it). Unlink is a `<details>/<summary>` two-step confirmation (no interactive `ConfirmModal` on a static SSR page); the OAuth challenge itself is a plain `<form>` posting to `POST /Account/PerformExternalLogin` with a hidden `returnUrl`, not a same-page handler. |
| `/Account/Privacy` | `Blazor/Pages/Account/Privacy.razor` (+ `.razor.cs`, `.razor.css`) | Static SSR, `[Authorize]`, default `MainLayout`. Replaces `Pages/Account/Privacy.cshtml` + `PrivacyModel` (cluster 4c) — calls `IConsentService`/`IUserDataExportService`/`IUserPurgeService` directly (no wrapper service; each mutation is already one call). Consent toggles are per-consent `<EditForm>`s pinned to the two `ConsentType` members that exist today (`FormName` needs a compile-time constant, which an open `@foreach` can't supply — extend the pair in `Privacy.razor.cs`, not the loop, if `ConsentType` grows). "Delete My Data" replaces the legacy typed-confirm JS dialog and `fetch('?handler=DeleteData')` JSON round trip with ordinary `DataAnnotations` validation (`Confirmation` must equal the literal text `DELETE`) — a mismatch never reaches the handler, so it renders inline via `ValidationMessage` on the same request; a real purge signs the session out (`SignInManager.SignOutAsync`) and redirects to `/landing` instead of returning JSON. Same `?status=<key>[&detail=...]` banner mechanism as `LinkDiscord`, with `detail` carrying the export-success message (record count, download URL, 7-day expiry) whole, since it is entirely dynamic. |
