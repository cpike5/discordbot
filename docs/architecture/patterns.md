...
| Folder | Contents |
| --- | --- |
| `Blazor/App.razor` | Static root document: `<!DOCTYPE html>`/`<head>` (fonts, `app.css`, pre-paint theme script, sidebar FOUC guard - copied verbatim from `Pages/Shared/_Layout.cshtml`), `<HeadOutlet />`, `<Routes />`, `blazor.web.js`. |
