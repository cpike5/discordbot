...
| `AssistantOptions` | `Assistant` | `AssistantServiceExtensions` | Grouped into nested sub-options: `Sampling` (model/tokens/temperature/timeout), `RateLimits` (rate limit + bypass role), `Messages` (question/response length, error text, retry), `Tools` (doc tools, prompt/doc paths, and the loop guard rails: `ToolExecutionTimeoutMs` default 10000 — the per-tool deadline, now actually enforced; `MaxToolResultChars` default 8000 — ceiling on one tool result entering conversation history, `0` disables; `DuplicateToolCallLimit` default 3 — identical calls allowed per run before further ones are refused without executing the tool, `0` disables), `Cost` (cost tracking, prompt caching), `Privacy` (consent, logging, retention). Historical flat keys (e.g. `Assistant:MaxTokens`) still bind via obsolete forwarding properties and take precedence over the nested key (e.g. `Assistant:Sampling:MaxTokens`) if both are set. |
