...
| `StatTile` | (new — pulled out of `DashboardWidgetViewModel.BodyContent`'s repeated "label over big number" fragment, Phase 4 cluster 4d) | `Label` (required), `Value` (required), `ValueClass` (text color utility, default `text-text-primary`), `Class` |
...
| `SettingField` | `Pages/Shared/_SettingField.cshtml` (model `Core.DTOs.SettingDto`) | `Setting` (required), `OnValueChanged` (`EventCallback<(string Key, string Value)>`) - dispatches to `Toggle`/`Select`/`TextInput` by `SettingDataType`, renders the "Restart Required" indicator via `Badge`, `Class`, `AdditionalAttributes` |
...
| `LoadingOverlay` | `_PageLoadingOverlay` / `PageLoadingOverlayViewModel` + `ILoadingState` | `Variant` (`SpinnerVariant`), `Size` (`SpinnerSize`), `Message`, `SubMessage`, `OnCancel`, `CancelText`, `Class`, `AdditionalAttributes` — subscribes to `ILoadingState.Changed` |
...
| `UserPreviewPopoverContent` | `_UserPreviewPopup` / `UserPreviewViewModel` | `Model`, `Class`, `AdditionalAttributes` |
| `GuildPreviewPopoverContent` | `_GuildPreviewPopup` / `GuildPreviewViewModel` | `Model`, `Class`, `AdditionalAttributes` |
...
| `VoiceChannelPanel` | `_VoiceChannelPanel` + `voice-channel-panel.js` / `VoiceChannelPanelViewModel` + `VoiceChannelInfo`/`NowPlayingInfo`/`QueueItemInfo` | `GuildId` (string, required — an invalid/unparseable id renders a disabled placeholder instead of throwing), `IsCompact`, `ShowNowPlaying`, `ShowProgress`, `AvailableChannels`, `Queue` (caller-seeded, like `ActivityFeed.Items` — not part of `AudioStatusDto`), `OnJoined`/`OnLeft`/`OnStopped`/`OnSkipped`, `Class`, `AdditionalAttributes` | `IDashboardAudioStatusService.GetCurrentAudioStatus` initial; `AudioConnectedEvent`/`AudioDisconnectedEvent`/`PlaybackStartedEvent`/`PlaybackProgressEvent`/`PlaybackFinishedEvent`/`QueueUpdatedEvent`/`VoiceChannelMemberCountUpdatedEvent` live, all guild-scoped; join/leave/stop/skip call `IAudioService`/`IPlaybackService` directly, catching and toasting (`IToastService`) any failure instead of faulting the circuit |
...
| Component | From partial(s) | Key parameters |
| --- | --- | --- |
| `VoiceSelector` | `_VoiceSelector` / `VoiceSelectorViewModel` + `voice-selector.js` | `Voices` (`IReadOnlyList<VoiceSelectorVoiceOption>`), `Value`/`ValueChanged`, `Placeholder`, `SearchPlaceholder`, `Class`, `AdditionalAttributes`. Full rewrite as a native combobox — locale grouping, collapsible groups, live search filter, and Up/Down/Home/End/Enter/Escape keyboard nav are all component state; "click outside to close" is a transparent backdrop element instead of `browser.js`'s `OnClickOutsideAsync` (out of this tier's interop scope). |
| `StyleSelector` | `_StyleSelector` / `StyleSelectorViewModel` + inline script | `SelectedVoice`, `Value`/`ValueChanged`, `Intensity`/`IntensityChanged`, `AvailableStyles` (`IReadOnlyList<StyleOption>`), `CapabilitiesLoader` (`Func<string, Task<VoiceCapabilities?>>`, called from `OnParametersSetAsync` when `SelectedVoice` changes — the page's replacement for the partial's `fetch('/api/portal/tts/voices/{voice}/capabilities')`), `OnEmphasisSupportChanged` (`EventCallback<bool>`, replacing the `window.emphasisToolbar_updateCapability` global call), `Class`, `AdditionalAttributes`. |
| `PresetBar` | `_PresetBar` / `PresetBarViewModel` + inline script | `Presets` (`IReadOnlyList<PresetButtonViewModel>`), `ActivePresetId`, `OnApply` (`EventCallback<PresetButtonViewModel>` — a clicked custom preset is synthesized into one, `Id` prefixed `custom-`), `CustomPresets` (`IReadOnlyList<CustomTtsPreset>`, a new record — persistence stays the page's job, the component only raises `OnSaveCustom`/`OnDeleteCustom`), `Class`, `AdditionalAttributes`. `window.prompt()`/`window.confirm()` (no Blazor equivalent) are replaced with an inline name field and a two-click delete confirm, respectively. |
| `ModeSwitcher` | `_ModeSwitcher` / `ModeSwitcherViewModel` | `Value`/`ValueChanged` (`TtsMode`), `Class`, `AdditionalAttributes`. `localStorage["tts_mode_preference"]` persistence goes through `BrowserInterop` — read once in `OnAfterRenderAsync` (`firstRender` guarded), written on every change. |
| `SsmlPreview` | `_SsmlPreview` / `SsmlPreviewViewModel` | `Ssml`, `PlainLength`, `StartCollapsed`, `ShowCharacterCount`, `OnCopy`, `Class`, `AdditionalAttributes`. The regex XML syntax highlighter is ported to C# (`SsmlSyntaxHighlighter`, HTML-encodes every literal piece before wrapping it in a `<span>`, rendered via `MarkupString`); copy goes through `BrowserInterop.CopyToClipboardAsync`. |
