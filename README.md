...
A Discord bot built with .NET 10 and Discord.NET that provides a foundation for managing Discord servers through slash commands, a REST API, and an admin UI (Razor Pages, being ported to Blazor). The system combines a hosted Discord bot service with a Web API and admin dashboard for management and monitoring.
